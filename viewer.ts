...
export class Viewer {
    ...
    loadFile (fileName, onSuccess: Function) {
      function getExt (fileName) {
        ...
        if (indexOfQueryParams >= 0) { fileName = fileName.substring(0, indexOfQueryParams) }
    ...
    lookAtSphere (sphere, setY: boolean = false) {
      if (setY) { this.camera.position.setY(sphere.center.y) }
    ...
    */
    ...
    resizeCanvas (force: boolean = false) {
      if (!this.settings.autoResize && !force) { return }
...
class ViewerInput {
    ...
    onKeyDown = (event) => {
      ...
      if (event.shiftKey) { speed *= this.settings.camera.controls.shiftMultiplier }
      if (event.altKey) { speed *= this.settings.camera.controls.altMultiplier }
    ...
    onMouseMove = (event) => {
      ...
      if (event.buttons & 2) { this.cameraController.panCameraBy(delta) } else { this.cameraController.rotateCameraBy(delta) }
    };
...
function toColor (c) {
  if (!isColor(c)) { throw new Error('Not a color') }
